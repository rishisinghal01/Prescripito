...
const Sidebar = ({ setisMenuopen, isMenuopen }) => {
  ...
  return (
    <div className={`print:hidden flex flex-col h-full min-w-72 md:min-w-80 p-5 border-r border-[#5f6fff]/20 dark:border-gray-800 bg-white/70 dark:bg-gray-900/70 backdrop-blur-md transition-all duration-500 rounded-tr-3xl shadow-[5px_0_30px_rgba(95,111,255,0.08)] max-md:absolute left-0 z-20 ${!isMenuopen && "max-sm:-translate-x-full "}`}>
      ...
      {/* --- Search Bar --- */}
      <div className='flex items-center gap-2 p-3 mt-5 border border-[#5f6fff]/40 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 shadow-sm focus-within:ring-2 focus-within:ring-[#5f6fff]/60 transition'>
        <img src={chatassets.search_icon} className='w-4 invert opacity-70 dark:invert-0' alt='search' />
        <input
          onChange={(e) => setsearch(e.target.value)}
          value={search}
          type='text'
          placeholder='Search conversations'
          className='text-xs placeholder:text-[#5f6fff]/70 dark:placeholder:text-gray-500 outline-none bg-transparent flex-1 text-gray-700 dark:text-gray-300'
        />
      </div>

      {/* --- Recent Chats --- */}
      {chats.length > 0 && (
        <p className='mt-6 text-sm text-[#5f6fff] dark:text-blue-400 font-semibold uppercase tracking-wide'>
          Recent Chats
        </p>
      )}
            ...
            return (
              <div
                ...
                className={`p-3 px-4 rounded-lg cursor-pointer flex justify-between items-center group transition-all duration-200 ${isActive
                    ? 'bg-[#5f6fff]/10 dark:bg-gray-800 border border-[#5f6fff] dark:border-gray-600'
                    : 'border border-transparent hover:border-[#5f6fff]/40 dark:hover:border-gray-700 hover:bg-[#f3f5ff] dark:hover:bg-gray-800'
                  }`}
              >
                <div>
                ...
                <img
                  ...
                  className='hidden group-hover:block w-4 invert opacity-80 hover:opacity-100 dark:invert-0 transition'
                  alt='delete'
                />
      ...
      >
        <img
          ...
          className='w-5 opacity-80 dark:opacity-100'
          alt=''
        />
  ...
  {user && (
    ...
    />
      ...
      <img onClick={() => setisMenuopen(false)}
        ...
        className='absolute top-3 right-3 w-5 h-5 cursor-pointer sm:hidden invert opacity-80 hover:opacity-100 dark:invert-0 transition'
        alt='close'
      />
    </div>
  )

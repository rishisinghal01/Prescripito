    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
        ...
        {reldoc.slice(0,5).map((item,idx)=>{
            return <motion.div 
                ...
                key={idx} onClick={()=>{navigate(`/appointment/${item._id}`);
             scrollTo(0,0)
            }} className='border border-[#C9D8FF] bg-white rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] hover:shadow-xl transition-all duration-500 shadow-sm flex flex-col'>
        ...
        <button onClick={()=>{navigate('/doctors');scrollTo(0,0)}} className='bg-[#EAEFFF] text-gray-600 px-12 py-3 rounded-full mt-10 hover:bg-[#d6e2ff] transition-all duration-300 font-medium'>View more</button>
    </div>
  )
}

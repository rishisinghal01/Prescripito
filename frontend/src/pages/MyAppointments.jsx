...
const MyAppointments = () => {
  ...
  const [currentChatInfo, setCurrentChatInfo] = useState({ appointmentId: null, doctorName: "" });
  const navigate = useNavigate();

  const openChat = (appointmentId, doctorName) => {
    setCurrentChatInfo({ appointmentId, doctorName });
    setIsChatOpen(true);
  };
  ...
  return (
    <div>
        ...
        {appointments.map((item, idx) => (
          <div key={idx} className="grid grid-cols-[1fr_3fr] gap-4 sm:flex sm:gap-6 py-2 border-b">
            ...
            <div className="flex flex-col gap-2 justify-end">
              ...
              {!item.cancelled && (
                <button 
                  onClick={() => openChat(item._id, item.docData.name)}
                  className="sm:min-w-48 py-2 border rounded border-indigo-500 text-indigo-500 hover:bg-indigo-500 hover:text-white transition-all shadow-sm font-medium flex justify-center items-center gap-2"
                >
      ...
      <ChatModal 
        ...
        doctorName={currentChatInfo.doctorName} 
      />
    </div>
  );
};
